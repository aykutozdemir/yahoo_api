# XML generation for Yahoo API
# Based off of example created by jan@biochemfusion.com April 2009.

addin_id = "com.crypto.api.Yahoo"
addin_version = "1.0.0"
addin_displayname = "Yahoo API"
addin_publisher_link = "https://github.com/aykutozdemir/yahoo_api"
...
add_manifest_entry(manifest_xml, 'uno-typelibrary;type=RDB', 'XYahooApi.rdb')
add_manifest_entry(manifest_xml, 'configuration-data', 'CalcAddIn.xcu')
add_manifest_entry(manifest_xml, 'uno-component;type=Python', 'YahooApi.py')
manifest_xml.write('</manifest:manifest> \n')
...
def define_function(xml_file, function_name, description, parameters):
	...
	xml_file.write('    <prop oor:name="CompatibilityName"><value xml:lang="en">AutoAddIn.Yahoo.' + function_name + '</value></prop>\n')
...
define_function(calcaddin_xml, \
	'YAHOOPRICE', 'Retrieve the latest price for a Yahoo Ticker', \
	[('symbol', 'The Yahoo Ticker(e.g., KRDMD.IS)')])

define_function(calcaddin_xml, \
	'YAHOOHIGHPRICE', 'Retrieve the daily high price for a Yahoo Ticker', \
	[('symbol', 'The Yahoo Ticker(e.g., KRDMD.IS)')])

define_function(calcaddin_xml, \
	'YAHOOLOWPRICE', 'Retrieve the daily low price for a Yahoo Ticker', \
	[('symbol', 'The Yahoo Ticker(e.g., KRDMD.IS)')])

define_function(calcaddin_xml, \
	'YAHOOVOLUME', 'Retrieve the daily volume for a Yahoo Ticker', \
	[('symbol', 'The Yahoo Ticker(e.g., KRDMD.IS)')])
